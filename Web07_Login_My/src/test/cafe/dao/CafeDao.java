...
public class CafeDao {
	...
	public int getCountTF(CafeDto dto) {
	...
	public List<CafeDto> getListTF(CafeDto dto){
		...
		try {
			...
			String sql = "SELECT *"
					+ " FROM"
					+ "     (SELECT result1.*, ROWNUM AS rnum"
					+ "      FROM (SELECT num,writer,title,viewCount,regdate"
					+ "            FROM board_cafe"
			...
			//반복문 돌면서 결과 값 추출하기 
			while (rs.next()) {
				//select 된 파일의 정보를 CafeDto 객체에 담고 
				CafeDto tmp=new CafeDto();
				tmp.setNum(rs.getInt("num"));
				tmp.setWriter(rs.getString("writer"));
				tmp.setTitle(rs.getString("title"));
